...
public class BankMain extends Application {
    ...
    private void handleLogin(String username, String password, String role) {
        ...
        if (role.equals("Manager")) {
            Manager manager = new Manager(username, password); // Create manager instance
    ...
    private void showManagerOptions() {
        ...
        // Read all text files in the project directory and add their usernames to the account list
        ...
        logoutButton.setOnAction(e -> {
            ...
            start(new Stage()); // Start a new instance of BankMain, bringing the user back to the login screen
        });
    ...
    private void addCustomer() {
        ...
        backButton.setOnAction(e -> showManagerOptions()); // Go back to Manager Options page

        // Add components to layout
        grid.getChildren().addAll(usernameLabel, usernameField, passwordLabel, passwordField, firstNameLabel, firstNameField, lastNameLabel, lastNameField, depositLabel, depositField, submitButton, errorLabel, backButton);
    ...
    private void createCustomer(String username, String password, String firstName, String lastName, String initialDeposit) {
    ...
    private void showCustomerOptions(String username) {
        ...
        vbox.getChildren().addAll(depositButton, withdrawButton, purchaseButton);

        // Logout button
        Button logoutButton = new Button("Logout");
        logoutButton.setOnAction(e -> {
            primaryStage.close(); // Close the current stage
            BankMain loginScreen = new BankMain(); // Create a new instance of BankMain
            loginScreen.start(new Stage()); // Start the new instance, bringing the user back to the login screen
        });
        vbox.getChildren().add(logoutButton);
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (line.startsWith("Total Balance:")) {
                    totalBalance = Double.parseDouble(line.substring("Total Balance: $".length()));
                    break;
                }
            }
            scanner.close();
    ...
    private void updateBalance(String username, double amount) {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                String line = scanner.nextLine();
                if (line.startsWith("Total Balance:")) {
                    double currentBalance = Double.parseDouble(line.substring("Total Balance: $".length()));
    ...
    private void showOnlinePurchaseScreen(String username) {
        ...
        purchaseButton.setOnAction(e -> {
            double totalCartAmount = calculateTotalCartAmount(shoesCheckBox.isSelected(), headphonesCheckBox.isSelected(),
                    laptopCheckBox.isSelected(), iphoneCheckBox.isSelected(), bagCheckBox.isSelected());
            ...
            if (totalCartAmount < 50) {
                ...
                confirmPurchase(username, finalAmount, fee, shoesCheckBox, headphonesCheckBox, laptopCheckBox, iphoneCheckBox, bagCheckBox);
            } else {
